...
const RequestsPage = () => {
  ...
  const canCreateRequest =
    permissions.resources?.Requests?.actions?.includes("create");
  const canUpdateRequest =
    permissions.resources?.Requests?.actions?.includes("update");
  const canDeleteRequest =
    permissions.resources?.Requests?.actions?.includes("delete");
  ...
  const {
    ...
    isLoading,
    error,
  } = useQuery(["requests"], () => fetchRequests(token), { enabled: !!token });
  ...
  const mutation = useMutation(
    async (requestData) => {
      if (editRequest) {
        ...
        return axios.put(
          `${API_BASE_URL}/api/requests/${editRequest.id}`,
          requestData,
          {
            headers: { Authorization: `Bearer ${token}` },
          }
    ...
    },
    {
      ...
      onError: (error) => {
        console.error(
          "❌ Error creating or updating request:",
          error.response?.data || error.message
        );
        showAlert(
          "Error!",
  ...
  const handleOpen = (request = null) => {
    ...
    setNewRequest(
      request
        ...
        : { typeOfRequest: "leave", description: "" }
    );
    setOpen(true);
  };
